...
import { TaskSchema, TaskUpdateSchema, TaskStatusSchema, SubtaskSchema } from '../schemas/task';
...
export async function createSubtask (req: CustomRequest, res: Response) {
  try {    
    const subtaskData: SubtaskData = req.body;
		const authorEmail = req.user?.email;

		const validatedTask = SubtaskSchema.parse(subtaskData);

    const newSubtask = await taskService.createSubtask(authorEmail, validatedTask);
...
export async function listSubtasks(req: CustomRequest, res: Response) {
  try {
    const { taskId, parentSubtaskId } = req.params;
    const subtasks = await taskService.listSubtasks(parseInt(taskId), parseInt(parentSubtaskId));
